...
#This program presents Bikeshare Data from three cities, Chicago, New York and Washington.
#Users may view descriptive statistics of this data filtered by city or as an aggregation of all cities.
#Users may also choose to filter data by month (January - June) and/or by day of the week (Sunday-Saturday).

#This interactive program requires user input for:
#   user's name,
#   data filter types (city, month, day),
#   request for raw_data (viewed in lines of five), and
#   request to repeat or end the program.

#Below is the dictionary of all data files available for search
#key= city and value= city.csv (data file).
#Additional city data files may be added to the dictionary here in " 'city':'file_name.csv' " format.
CITY_DATA = { 'chicago': 'chicago.csv',
              'new york': 'new_york_city.csv',
              'washington': 'washington.csv' }


#Below are interactive questions for the user to provide user name and filter information for dataframes.
#This will return city, month, day.
def get_filters():
    """
...
#The following code loads CITY_DATA.csv files and converts them to filtered dataframes based on the
#city, month, day perameters returned by def get_filters
def load_data(city, month, day):
    ...
    #Reads and combines all files in CITY_DATA dictionary into one dataframe based on the 'All' filter for city
    else:
        i=0
        df_files = []
    ...
    # filter by month and create new dataframe, if applicable
    if month != 'All':
    ...
    # filter by day of week and create new dataframe, if applicable
    if day != 'All':
...
def time_stats(df,month,day):
    ...
    #Displays line chart of total number of users per month when 'month' =='All'
    if month == 'All':
    ...
    #Displays the month with the hightest number of users
    print('\nThe busiest month was:  ',calendar.month_name[(df['Month'].mode()[0])])

    #Displays line chart of total number of users per day when 'day' =='All'
    if day == 'All':
    ...
    #Displays line Chart of total usage over 24 hours
...
def station_stats(df):
    ...
    #Dsisplay most frequent combination of start station and end station trip
...
def user_stats(df):
    ...
    #Displays bar chart of users based on gender, if gender column is included in data
    ...
    #Displays counts of gender, if data available
...
#Displays 5 lines of raw data from filtered dataframe based on user input (yes/no)
